...
def get_arguments(argv):
    ...
    parser.add_argument(
        "-r",
        "--regex-exclusion",
        help="Regular expression to exclude columns from keys",
    )
    parser.add_argument(
        "-s",
        "--max-key-size",
        help="Maximum key size to accept",
        type=int,
        default=None,
    )
    parser.add_argument(
...
def main(argv):
    ...
    max_length = (
        args.max_key_size
        if args.max_key_size and args.max_key_size < len(header)
        else len(header)
    )
    all_combinations = chain.from_iterable(
        combinations(header, r=i) for i in range(1, max_length + 1)
    )
